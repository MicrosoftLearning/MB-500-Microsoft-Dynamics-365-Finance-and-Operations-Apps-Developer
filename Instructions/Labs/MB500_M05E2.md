...
2.  If you do not find the project **MB500TrainingProjectCode:**
    ...
    5.  Select **Add** (or **Create**). Set the new project’s **Model** property to **MB500Training** if available. Otherwise, once created, right click, select Properties, and set the model there.
...
19. Enter **MBTGeneratePremiums** in the **Object field**.

20. Enter **Generate premium** in the **Label** field.

21. **Right click** on **MB500TrainingProjectCode** and select **Build**.

22. **Click** on **MB500TrainingProjectCode**.

23. **Open** Application Explorer (from the View menu, if not already visible).

24. Go to **AOT \> User Interface \> Forms**.

25. **Search** and select **MBTAssetInsuranceDetails**.

26. **Right click** on **MBTAssetInsuranceDetails** and select **Add to
    project**.

27. Make sure MBTAssetInsuranceDetails form is added under the project
    MB500TrainingProjectCode.
...
31. **Create** a new **ButtonGroup**, **ActionButtonGroup**. (Note: If you imported the code at the beginning of the lab, it may already be there.)
...
1.  Navigate to the Dynamics 365 Finance and Operations apps environment in the
    browser. **Refresh** if it was already open, as the build will have broken connections.
...
-  The lab resources can be found by opening the **File explorer** located on the **Taskbar**. then selecting the **Documents** folder and finally selecting the **Course Assets** folder. If your Authorized Lab Hoster has not provided the files, you may find them at and download them from https://aka.ms/mb500resources/
...
-   Note that if you **Build** the solution, you will get an error for a menu item, which we will take care of in this exercise.
